document.addEventListener('DOMContentLoaded', () => {
  ...
  const resRéaumur = document.getElementById('res-reaumur');
  ...
  document.querySelectorAll('.toggle-dropdown-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (targetEl.style.display === 'none') {
        targetEl.style.display = targetEl.classList.contains('extra-grid') ? 'grid' : 'block';
  ...
  // Independent Dark/Light Mode Toggle (Sun click -> Light mode, Moon click -> Normal Dark mode)
  darkModeBtn.addEventListener('click', () => {
    document.body.classList.toggle('light-mode');
    const icon = darkModeBtn.querySelector('i');

    // Agar light-mode class aa gayi (Matlab white page active hai), toh Moon icon dikhega
    if (document.body.classList.contains('light-mode')) {
      icon.className = 'fa-solid fa-moon';
    } else {
      // Agar normal dark page hai, toh Sun icon dikhega
      icon.className = 'fa-solid fa-sun';
    }
  });

  // Independent Theme Color Palette Switcher
